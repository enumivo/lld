...
void Writer::createOutputSegments() {
  for (ObjFile *File : Symtab->ObjectFiles) {
    if (!File->getEnumivoABI().empty())
       abis.push_back(File->getEnumivoABI());
...
static constexpr uint64_t ENUMIVO_COMPILER_ERROR_BASE = 8000000000000000000ull;
static constexpr uint64_t ENUMIVO_ERROR_NO_ACTION     = ENUMIVO_COMPILER_ERROR_BASE;
static constexpr uint64_t ENUMIVO_ERROR_ONERROR       = ENUMIVO_COMPILER_ERROR_BASE+1;
...
void Writer::createDispatchFunction() {
   ...
   auto create_if = [&](raw_string_ostream& os, std::string str, bool& need_else) {
      ...
      need_else = true;
      uint64_t nm = enumivo::cdt::string_to_name(str.substr(0, str.find(":")).c_str());
   ...
   auto assert_sym = (FunctionSymbol*)Symtab->find("enumivo_assert_code");
   ...
   auto create_action_dispatch = [&](raw_string_ostream& OS) {
      ...
      for (ObjFile *File : Symtab->ObjectFiles) {
        if (!File->getEnumivoActions().empty()) {
            for (auto act : File->getEnumivoActions()) {
      ...
      // do not fail if self == enumivo
      writeU8(OS, OPCODE_GET_LOCAL, "GET_LOCAL");
      writeUleb128(OS, 0, "self");
      writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
      encodeSLEB128((int64_t)enumivo::cdt::string_to_name("enumivo"), OS);
      writeU8(OS, OPCODE_I64_NE, "I64.NE");
      writeU8(OS, OPCODE_IF, "if receiver != enumivo");
      writeU8(OS, 0x40, "none");
      ...
      if (assert_sym && assert_idx < Symtab->getSymbols().size()) {
        ...
        writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
        encodeSLEB128((int64_t)ENUMIVO_ERROR_NO_ACTION, OS);
        writeU8(OS, OPCODE_CALL, "CALL");
   ...
   auto create_notify_dispatch = [&](raw_string_ostream& OS) {
      ...
      for (ObjFile *File : Symtab->ObjectFiles) {
         if (!File->getEnumivoNotify().empty()) {
            for (auto notif : File->getEnumivoNotify()) {
      ...
      if (!has_onerror_handler) {
         // assert on onerror
         writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
         uint64_t acnt = enumivo::cdt::string_to_name("enumivo");
         encodeSLEB128((int64_t)acnt, OS);
         writeU8(OS, OPCODE_GET_LOCAL, "GET_LOCAL");
         writeUleb128(OS, 1, "code");
         writeU8(OS, OPCODE_I64_EQ, "I64.EQ");
         writeU8(OS, OPCODE_IF, "IF code==enumivo");
         writeU8(OS, 0x40, "none");
         writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
         uint64_t nm = enumivo::cdt::string_to_name("onerror");
         encodeSLEB128((int64_t)nm, OS);
         writeU8(OS, OPCODE_GET_LOCAL, "GET_LOCAL");
         writeUleb128(OS, 2, "action");
         writeU8(OS, OPCODE_I64_EQ, "I64.EQ");
         ...
         writeUleb128(OS, 0, "false");
         writeU8(OS, OPCODE_I64_CONST, "I64.CONST");
         encodeSLEB128((int64_t)ENUMIVO_ERROR_ONERROR, OS);
         writeU8(OS, OPCODE_CALL, "CALL");
         writeUleb128(OS, assert_idx, "code");
         writeU8(OS, OPCODE_END, "END");
         writeU8(OS, OPCODE_END, "END");
      ...
      if (not_cnt > 0) {
         for (auto const& notif0 : notify_handlers) {
            uint64_t nm = enumivo::cdt::string_to_name(notif0.first.c_str());
            if (notif0.first == "*")
               continue;
